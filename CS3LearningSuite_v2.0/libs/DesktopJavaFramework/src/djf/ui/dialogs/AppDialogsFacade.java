...
public class AppDialogsFacade {
    ...
    public static void showExportDialog(AppTemplate app) throws IOException {
        AppWebDialog dialog = new AppWebDialog(app);
        PropertiesManager props = PropertiesManager.getPropertiesManager();
        String exportPath = props.getProperty(APP_PATH_EXPORT_DATA);
        String filePath = exportPath + props.getProperty(APP_EXPORT_PAGE);
        dialog.showWebDialog(filePath);
    }
    ...
    public static File showImageInputDialog(Stage window, AppPropertyType saveTitleProp) {
        // PROMPT THE USER FOR A FILE NAME
        FileChooser fc = new FileChooser();
        fc.setInitialDirectory(new File(PATH_IMAGE));
        PropertiesManager props = PropertiesManager.getPropertiesManager();
        fc.setTitle(props.getProperty(saveTitleProp));
        fc.getExtensionFilters().addAll(
                new FileChooser.ExtensionFilter(props.getProperty(IMAGE_FILE_EXT_DESC), props.getProperty(IMAGE_FILE_PNG_EXT)),
                new FileChooser.ExtensionFilter(props.getProperty(IMAGE_FILE_EXT_DESC), props.getProperty(IMAGE_FILE_JPG_EXT))
        );
        File selectedFile = fc.showOpenDialog(window);
        return selectedFile;
    }
