...
public class AppFileController {
    ...
    public void processExportRequest() {
        ...
        }catch(NoPageSelectedException npse) {
            AppDialogsFacade.showMessageDialog(app.getGUIModule().getWindow(), NO_PAGE_SELECTED_TITLE, NO_PAGE_SELECTED_CONTENT);
        }catch (NullPointerException npe) {
            processSaveRequest();
            if (fileSettings.getWorkFile() != null)
                processExportRequest();
        } catch (Exception ex) {
            AppDialogsFacade.showStackTraceDialog(app.getGUIModule().getWindow(), ex, EXPORT_ERROR_TITLE, EXPORT_ERROR_CONTENT);
        }
    }
